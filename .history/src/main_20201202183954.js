...
router.beforeEach((to, from, next) => {
  ...
  if (isLoadNodes) {
    // let asyncRoute = store.dispatch('getLastRoute')
    store.dispatch('getLastRoute').then( res => {
      console.log("123456" + JSON.stringify(router.options.routes))
  ...
  // 针对自定义路由页面刷新的情况(刷新时to.name为null),刷新后store中的数据均清空 需要重新获取
  if (!to.name) {
