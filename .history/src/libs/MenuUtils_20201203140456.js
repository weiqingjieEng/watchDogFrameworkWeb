  ...
  data.forEach((item) => {
    if (item.type ===0 &&item.name!=='首页') {
      ...
      if (item.parentId === null) { // 如果是父节点
        ...
        menu.name = item.children!== null? item.permission+':list' : item.permission
        let smeta = {icon: (item.icon === ''|| item.icon === null)?'test_456': item.icon, title: item.name, access: [item.children!== null? item.permission+':list' : item.permission]}
        menu.meta = smeta
