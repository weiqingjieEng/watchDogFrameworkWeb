...
export default {
   ...
   methods: {
     ...
     async show (ev) {
      ...
      try {
        let res = await post(this.selectData,{selectParamKey: this.searchText}) // 获取模糊查询的数据
    ...
    handleSubmit: function (searchText) { // 点击查询时，根据输入框searchText遍历 result获取图表参数对应的值 生成object 返回至父组件
      let searchDTO = ''
      let searchKey = this.searchDTO
      this.result.forEach((item, index) => {
        if (searchText === item.name) {
          searchDTO = item.DTO
        }
      })
      let param = { searchKey: searchDTO}
      this.$emit('returnSearch',{ 'url': this.searchData, 'param': param })
    }
   },
   watch: {
