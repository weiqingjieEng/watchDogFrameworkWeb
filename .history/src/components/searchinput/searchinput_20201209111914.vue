<template>
  <div class="searchInput">
    <Row>
      <Col span="8" push="4">
        <!-- <Input :spellcheck='true' v-model="searchText" @keyup.native='show($event)' @keydown.down.prevent.native='down()' @keydown.up.prevent.native='up()' placeholder="请输入搜索条件...">
        ...
        <input v-model="searchText" @keyup ='show($event)' @keydown.down.prevent ='down()' @keydown.up.prevent='up()' placeholder="请输入搜索条件..."/>
...
export default {
   ...
   methods: {
     ...
     show: function (ev) {
      ...
      console.log('查询数据'+ this.searchText)
      // try {
    ...
    async getList (param) {
      try {
        let res = await post(this.selectData, param)
        if (res.status ===1 && res.data.length!== 0) {
