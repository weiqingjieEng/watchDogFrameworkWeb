<template>
  <div class="searchInput">
    <Row>
      <Col span="8" push="8">
        <Input v-model="searchText" @keyup.native='show($event)'>
...
export default {
   ...
   methods: {
     ...
     show: function (ev) {
      ...
      console.log(this.selectText +'----'+ this.searchText)
      if (this.searchText) {
        console.log(this.searchText)
      }
      // try {
      //   post(this.selectData,{selectParamKey: this.searchText}).then((res) => {
      //     if (res.status ===1) {
      ...
      //   this.$throw(error)
      // }
      // this.$http.jsonp(this.selectData, {
