              ...
              obj[prop][proitem].forEach(element => {
                if (isSource=== 0) {
                  links.push({
                    ...
                    linkDetail:{source: sourceName,target: element['companyName'],relation:element.partnerCount!==undefined?'合作次数'+element.partnerCount:'投标'}
                  })
                }else if(isSource=== 1) {
                  links.push({
                    ...
                    label: { align: 'center', fontSize: 12 },
                    lineStyle: { color: category>Color.length?Color[category]:'#000000', curveness: curveness },
                    linkDetail:{source: element['companyName'],target: sourceName,relation:element.partnerCount!==undefined?'合作次数'+element.partnerCount:'投标'}
                  })
                }
                category++
