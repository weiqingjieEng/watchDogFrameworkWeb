      ...
      for (let prop in obj){
        if (typeof (obj[prop]) === 'object' && obj[prop] !== null) {
          ...
          for (let proitem in obj[prop]) {
              ...
              obj[prop][proitem].forEach(element => {
                if (isSource=== 0) {
                  links.push({
                    ...
                    name: element.partnerCount!==undefined?'合作次数:'+element.partnerCount:'投标' ,
                    label: { align: 'center', fontSize: 12 },
                    lineStyle: { color: category>Color.length?Color[category]:randomColor().color, curveness: curveness }
                  })
                }else if(isSource=== 1) {
                  links.push({
            ...
            // if(typeof obj[prop][proitem] === 'object' && proitem !== null) {
            //   category++
            //   this.getLinks(obj[prop]['companyName'], links, Color, category, obj[prop][proitem])
            // }
          }
        }
      }
