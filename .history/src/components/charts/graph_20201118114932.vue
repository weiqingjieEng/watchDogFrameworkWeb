...
export default {
  ...
  methods: {
    ...
    arrangedata:function () {
      ...
      let companyNodes = {name: this.sourcedata.companyName, category: category, symbolSize: randomSize(30, 60),itemStyle: { color: randomColor()} }
      ...
      for (let prop in this.sourcedata){
        if (typeof (_this.sourcedata[prop]) === 'object' && _this.sourcedata[prop] !== null) {
          ...
          let keyNameFir = 'provinceName' // 根据不同类型获取不同数组里的name
          let keyNameSec = 'cityName'
          if (prop === 'bidDepts') {
            keyNameFir = 'company'
            keyNameSec = 'deptName'
          } else if (prop === 'bidPersons') {
            keyNameFir = 'companyList'
            keyNameSec = 'personName'
          }
          for (let item in _this.sourcedata[prop].datas){
            let nodeitem ={}
            nodeitem.name = _this.sourcedata[prop].datas[item][keyNameFir] + _this.sourcedata[prop].datas[item][keyNameSec]
