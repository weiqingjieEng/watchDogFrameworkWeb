...
export default {
  ...
  methods: {
    async getdata () {
      ...
      // var changesoption=graphChart.getOption();
      // changesoption.series[0].data[params.dataIndex].x=params.event.offsetX;
      // changesoption.series[0].data[params.dataIndex].y=params.event.offsetY;
      // changesoption.series[0].data[params.dataIndex].fixed=true;
      // graphChart.setOption(changesoption);
      // });
              ...
              obj[prop][proitem].forEach(element => {
                if (isSource=== 0) {
                  links.push({
                    ...
                    detailData: {type: element.partnerCount!==undefined?'合作关系':'投标关系'}
                  })
                }else if(isSource=== 1) {
                  links.push({
