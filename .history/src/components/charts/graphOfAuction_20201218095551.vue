    ...
    getNodeAndLinks:function(nodes, category, Color, symbolSize, keyName, categoryName, linkName, curveness, obj){
      for (let prop in obj) {
        if (obj[prop] instanceof Array && obj[prop] !== null) {
          obj[prop].array.forEach(element => {
            let nodeitem = {}
            nodeitem.name = element[keyName]
            nodeitem.category = category
            nodeitem.symbolSize = symbolSize
            nodeitem.itemStyle = { color: Color[category]}
            nodeitem.detailData = this.getDetailData(element,categoryName)
            nodes.push(nodeitem)
            for (let prop in element){
              if (typeof (element[prop]) === 'object' && element[prop] !== null) {
                this.getNodeAndLinks(nodes, category, Color, symbolSize, keyName, categoryName, linkName, curveness, element[prop])
              }
            }
          });
