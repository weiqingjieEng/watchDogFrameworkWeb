...
export default {
  ...
  methods: {
    ...
    arrangedata:function () {
      ...
      let companyNodes = {name: this.sourcedata.companyName, category: category, symbolSize: randomSize(50, 80),itemStyle: { color: companyColor.color},label: {color: companyColor.textcolor} }
      ...
      for (let prop in this.sourcedata){
        if (typeof (_this.sourcedata[prop]) === 'object' && _this.sourcedata[prop] !== null) {
          ...
          for (let item in _this.sourcedata[prop].datas){
            ...
            nodeitem.label = {color: companyColor.textcolor}
            nodes.push(nodeitem)
