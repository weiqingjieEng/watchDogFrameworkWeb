<template>
  <div class="graphContent">
    ...
    <ul :style='{width:graphwidth+"%",height:graphheight+"%", padding:"40px 10px"}' v-show='relation.length !== 0'>
    ...
    <div class="nodeshow" v-if="Object.keys(detailData).length!==0" :style='{width:graphwidth+"%",height:graphheight+"%", padding:"40px 10px"}'>
      ...
      <div v-if="detailData.type==='公司'">
        <ul>
          <li>公司名称:&nbsp;{{detailData.companyName}}</li>
          <li>公司地址:&nbsp;{{detailData.companyAddress}}</li>
        </ul>
      </div>
      <div v-else-if="detailData.type==='投标区域'">
        <ul>
          <li>投标区域:&nbsp;{{detailData.country}}·{{detailData.provinceName}}·{{detailData.cityName}}</li>
        </ul>
      </div>
      ...
      <div v-else-if="detailData.type==='发包人'">
        <li>发包人:&nbsp;{{personNamedetailData.personName}}</li>
        <li>联系方式:&nbsp;{{'tel'in detailData?detailData.tel:'暂无'}}</li>
        <li>邮箱:&nbsp;{{'email' in detailData?detailData.email:'暂无'}}</li>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    arrangedata:function () { // 整理数据 后期还需根据数据进行修改
      ...
      let firNodes = {
        ...
        detailData: this.getDetailData(this.sourcedata,this.graphparam.searchByKey==='personName'?'发包人':(this.graphparam.searchByKey==='deptName'?'发包单位':(this.graphparam.searchByKey==='companyName'?'公司':'')))
        }
      let firCategory = {
