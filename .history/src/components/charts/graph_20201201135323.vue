  ...
  <div class="graphContent">
    ...
    <div class="nodeshow" v-if="Object.keys(detailData).length!==0" :style='{width:graphwidth+"%",height:graphheight+"%", padding:"40px 10px"}'>
      ...
      <div v-if="detailData.type==='公司'">
        <ul>
          <li>公司名称:&npsb{{detailData.companyName}}</li>
        </ul>
      </div>
      <div v-else-if="detailData.type==='投标区域'"></div>
      <div v-else-if="detailData.type==='发包单位'"></div>
      <div v-else-if="detailData.type==='发包人'"></div>
    </div>
  </div>
</template>
<script>
