...
export default {
  ...
  methods: {
    ...
    arrangedata:function () { // 整理数据 后期还需根据数据进行修改
      ...
      let firCategory = {
        name: this.graphparam.searchByKey==='personName'?'发包人':(this.graphparam.searchByKey==='deptName'?'发包单位':(this.graphparam.searchByKey==='bidCompany'?'公司':'')),
        category: category, 
        itemStyle: { color: Color[category] }}
      nodes.push(firNodes)
        ...
        if (typeof (_this.sourcedata[prop]) === 'object' && _this.sourcedata[prop] !== null) {
          ...
          for (let item in _this.sourcedata[prop].datas){
            ...
            if (_this.sourcedata[prop].isSource=== 0) {
              links.push({
                source: _this.sourcedata[this.graphparam.searchByKey],
              ...
              links.push({
                ...
                target: _this.sourcedata[this.graphparam.searchByKey],
                name: categoryName ,
                label: { align: 'center', fontSize: 12 },
                lineStyle: { color: Color[category], curveness: curveness }
              })
