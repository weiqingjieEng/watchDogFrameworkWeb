      ...
      for (let prop in obj){
        if (typeof (obj[prop]) === 'object' && obj[prop] !== null) {
          ...
          for (let proitem in obj[prop]) {
            if (proitem instanceof Array) {
              proitem.forEach(element => {
                if (isSource=== 0) {
                  links.push({
                    source: sourceName,
                    target: element[targetName],
                    name: element.partnerCount!==undefind?'合作次数:'+element.partnerCount:'投标' ,
                    label: { align: 'center', fontSize: 12 },
                    lineStyle: { color: Color[category], curveness: curveness }
                  })
                }else if(isSource=== 1) {
                  links.push({
                    source: element[targetName],
                    target: sourceName,
                    name: element.partnerCount!==undefind?'合作次数:'+element.partnerCount:'投标' ,
                    label: { align: 'center', fontSize: 12 },
                    lineStyle: { color: Color[category], curveness: curveness }
            ...
            if(typeof proitem === 'object' && proitem !== null) {
              category++
              this.getLinks(obj[prop]['companyName'], links, Color, category, proitem)
            }
          }
        }
      }
