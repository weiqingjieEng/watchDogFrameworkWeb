...
export default {
  ...
  methods: {
    ...
    arrangedata:function () { // 整理数据 后期还需根据数据进行修改
      ...
      for (let prop in this.sourcedata){
        if (typeof (_this.sourcedata[prop]) === 'object' && _this.sourcedata[prop] !== null) {
          ...
          //   nodes.push(nodeitem)
          // }
          this.getNodeAndLinks(nodes, category, keyName, categoryName, linkName, curveness,_this.sourcedata[prop])
        }
      }
    ...
    getNodeAndLinks:function(nodes, category, keyName, categoryName, linkName, curveness, obj){
      for (let prop in obj) {
          ...
          for (let item in obj[prop]) {
            ...
            for (let prop in nodeitem){
              if (typeof (nodeitem[prop]) === 'object' && nodeitem[prop] !== null) {
                this.getNodeAndLinks(nodes, category, keyName, categoryName, linkName, curveness, nodeitem[prop])
              }
            }
          }
        }
