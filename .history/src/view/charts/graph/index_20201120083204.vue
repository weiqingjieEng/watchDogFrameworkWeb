<template>
  <div class="graphcontainer">
    <!-- <Col span="7">
      <FormItem label="公司名称">
          <Input type="text" v-model="formInline.company" @keyup.native='show($event)' @keydown.down.prevent.native='down()' @keydown.up.prevent.native='up()' placeholder="请输入公司名称...." style="width:300px"/>
          <ul class="inputUl">
    ...
    <searchinput :selectitem="selectitem"></searchinput>
...
export default {
  data () {
    return {
      ...
      graphheight1: 100,
      graphname1: 'company',
      graphurl1: '/graph/bidCompany/get',
      formInline: {
        company: '',
        user: '',
        pdu: ''
  ...
  methods: {
    ...
    show: function(ev) {
      if (ev.keyCode === 38 || ev.keyCode === 40) {
        ...
        if (this.nowIndex != this.result.length && this.nowIndex != -1) {
          this.formInline.company = this.result[this.nowIndex].name
        }
        return
      }
      if (ev.keyCode === 13) {
      ...
      if (ev.keyCode === 8 &&this.formInline.company=== '') {
        this.result =[]
      }
      ...
      this.allcompany.forEach(function (item, index) {
        if (item.name.toLowerCase().indexOf(_this.text.toLowerCase()) > -1) {
    ...
    getClickData: function(ev) {
      let _this = this
      this.result.forEach((item, index) => {
        if (item.name === ev.srcElement.innerText) {
          _this.formInline.company = _this.result[index].name
          _this.result =[]
          return false
        }
      })
