    ...
    <div class="formcontainer">
      <Form ref="formInline" :model="formInline" :label-width="100" inline>
        <Row>
          <Col span="7">
            <FormItem label="公司名称">
                <Input type="text" v-model="formInline.company" @keyup.enter.native='show($event)' @keyup.native='down()' @keydown.native='up()' placeholder="请输入公司名称...." style="width:300px"/>
                <ul>
  ...
  methods: {
    ...
    show: function(ev) { 
      ...
      this.allcompany.forEach(function (item, index) {
        if (item.name.toLowerCase().indexOf(_this.formInline.company.toLowerCase()) > 0) {
